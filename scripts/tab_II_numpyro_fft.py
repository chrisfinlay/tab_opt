...
def pow_spec(k, P0=1e7, k0=3e-3, gamma=2):
    return P0 / ((1.0 + (jnp.abs(k) / k0) ** 2) ** (gamma / 2))
...
def model(params):
    ...
    G_phase = numpyro.deterministic(
        "g_phase", jnp.concatenate([G_phase, jnp.zeros((1, 1))], axis=0)
    )

    G = numpyro.deterministic(
        "gains", G_amp * jnp.exp(1.0j * G_phase) * jnp.ones((n_ant, n_time))
    )
    ...
    rfi_k_r = Normal(
        "rfi_k_r",
        jnp.zeros((n_rfi_k, n_ant)),
        rfi_Pk_root[:, None] * jnp.ones((n_rfi_k, n_ant)),
    )
    rfi_k = numpyro.deterministic(
        "rfi_k", vmap(construct_real_fourier, in_axes=(1,))(rfi_k_r).T
    )

    rfi_vis = numpyro.deterministic(
        "rfi_vis", get_rfi_vis_fft1(rfi_k, a1, a2, params["rfi_k_kernel"])
    )
    ...
    ast_k_r = Normal(
        "ast_k_r",
        jnp.zeros((n_ast_k, n_bl)),
        ast_Pk_root[:, None] * jnp.ones((n_ast_k, n_bl)),
    )
    ast_k_i = Normal(
        "ast_k_i",
        jnp.zeros((n_ast_k, n_bl)),
    ...
    ast_k = numpyro.deterministic("ast_k", ast_k_r + 1.0j * ast_k_i)

    ast_vis = numpyro.deterministic(
        "ast_vis", vmap(fft_inv_even, in_axes=(1, None, None))(ast_k, N_pad_ast, NN_ast)
    )
    # ast_vis = numpyro.deterministic('ast_vis', jnp.fft.ifft(ast_k, axis=0).T/dt)

    vis_obs = numpyro.deterministic(
        "vis_obs", G[a1] * jnp.conjugate(G[a2]) * (ast_vis + rfi_vis)
    )
    vis_obs = jnp.concatenate([vis_obs.real, vis_obs.imag], axis=1)

    numpyro.deterministic(
        "rmse_ast",
        jnp.sqrt(jnp.mean(jnp.abs((ast_vis - params["vis_ast_true"])) ** 2, axis=1)),
    )

    return numpyro.sample(
        "obs", dist.Normal(vis_obs, params["noise"]), obs=params["vis_obs"].T
    )
...
print(f"RFI Parameter Ratio : {nn/121:.2f}")

vis_rfi_test = (rfi_k_kernel * rfi_kI_[None, :, :]).sum(axis=1)

rfi_k1 = vmap(construct_real_fourier, in_axes=(1,))(
    jnp.concatenate([rfi_k_[: nn // 2 + 1].real, rfi_k_[1 : nn // 2].imag], axis=0)
).T
...
true_params = {
    ...
    **{"g_phase_": jnp.angle(G).mean(axis=1)[: N_ant - 1, None]},
    ...
    # **{'rfi_k_i': rfi_k_.imag},
    **{
        "rfi_k_r": jnp.concatenate(
            [rfi_k_[: nn // 2 + 1].real, rfi_k_[1 : nn // 2].imag], axis=0
        )
    },
