...
from tab_opt.models import (
    ...
    fixed_orbit_rfi_all_fft_standard_model,
)
...
### Define Model
vis_model = fixed_orbit_rfi_all_fft_standard_model
...
model_name = "fixed_orbit_rfi_all_fft_standard"
...
@jit
def ast_pow_spec(k, P0=1e3, k0=1e-3, gamma=1.0):
    k_ = (k / k0) ** 2
...
@jit
def rfi_pow_spec(k, P0=1e7, k0=3e-3, gamma=2):
...
print()
print(
    f"Number of parameters : {((2 * N_ant - 1) * n_g_times) + (2 * N_time * N_bl) + (2 * N_ant * nn)}"
)
...
rfi_phase = get_rfi_phase(times_fine, rfi_orbit, ants_uvw, ants_xyz, freqs).T
...
rfi_pow_spec_args = {"P0": 1.5e3, "k0": 3e-3, "gamma": 2}
...
args.update(
    {
        ...
        "mu_rfi_r": jnp.zeros((N_ant, nn)),
        "mu_rfi_i": jnp.zeros((N_ant, nn)),
        ...
        "sigma_ast_k": jnp.array(
            [ast_pow_spec(k_ast, **pow_spec_args) for _ in range(N_bl)]
        ),
        "sigma_rfi_k": rfi_pow_spec(k_, **rfi_pow_spec_args),
...
def inv_transform(params, loc, inv_scaling):
    params_trans = {
        "rfi_r_induce_base": vmap(affine_transform_diag_inv, in_axes=(0, None, 0))(
            params["rfi_r_induce"], inv_scaling["sigma_rfi_k"], loc["mu_rfi_r"]
        ),
        "rfi_i_induce_base": vmap(affine_transform_diag_inv, in_axes=(0, None, 0))(
            params["rfi_i_induce"], inv_scaling["sigma_rfi_k"], loc["mu_rfi_i"]
        ),
...
inv_scaling = {
    "sigma_rfi_k": 1.0 / args["sigma_rfi_k"],
