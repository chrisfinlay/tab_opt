...
from tab_opt.models import (
    fixed_orbit_real_rfi_compressed3,
)
...
### Define Model
model = fixed_orbit_real_rfi_compressed3
model_name = "fixed_orbit_real_rfi_compressed3"
...
args.update(
    {
        "mu_G_amp": [x for x in jnp.abs(gains_induce)],
        "mu_G_phase": [x for x in jnp.angle(gains_induce[:-1])],
        # "mu_rfi_r": true_params["rfi_r_induce"],
        # "mu_rfi_r": jnp.zeros_like(rfi_induce.real),
        "mu_rfi_r": [jnp.zeros((n_rfi_times,)) for _ in range(N_ant)],
        # "mu_vis_r": [true_params[f"vis_r_induce_{i}"] for i in range(N_bl)],
        # "mu_vis_i": [true_params[f"vis_i_induce_{i}"] for i in range(N_bl)],
        "mu_vis_r": [jnp.zeros(n_t) for n_t in n_vis_times],
        "mu_vis_i": [jnp.zeros(n_t) for n_t in n_vis_times],
        "L_G_amp": N_ant
        * [
            jnp.linalg.cholesky(kernel(g_times, g_times, g_amp_var, g_l)),
        ],
        "L_G_phase": (N_ant - 1)
        * [
            jnp.linalg.cholesky(kernel(g_times, g_times, g_phase_var, g_l)),
        ],
        # "L_vis": {
        ...
        "L_RFI_amp": N_ant
        * [
            jnp.linalg.cholesky(kernel(rfi_times, rfi_times, rfi_var, rfi_l)),
        ],
        # "resample_vis": {
        ...
        # },
        "resample_vis": [
            resampling_kernel(vis_ast_times[i], times, vis_var[i], vis_l[i], 1e-3)
            for i in range(N_bl)
        ],
...
def transform_params(x, loc, scale_tril):
    x_trans = tree_map(
        lambda x, loc, scale_tril: scale_tril @ x + loc, x, loc, scale_tril
    )
    return x_trans


param_scale_tril = {
    **{f"g_amp_induce_{i}": L for i, L in enumerate(args["L_G_amp"])},
    **{f"g_phase_induce_{i}": L for i, L in enumerate(args["L_G_phase"])},
    **{f"rfi_r_induce_{i}": L for i, L in enumerate(args["L_RFI_amp"])},
    **{f"vis_r_induce_{i}": L for i, L in enumerate(args["L_vis"])},
    **{f"vis_i_induce_{i}": L for i, L in enumerate(args["L_vis"])},
}

param_scale = tree_map(jnp.diag, param_scale_tril)
