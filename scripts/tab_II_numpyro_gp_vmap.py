...
from tab_opt.models import (
    fixed_orbit_real_rfi_compressed4,
)
...
### Define Model
model = fixed_orbit_real_rfi_compressed4
model_name = "fixed_orbit_real_rfi_compressed4"
...
print(f"AST   : {n_vis_times: 4}")
print()
print(
    f"Number of parameters : {((2*N_ant-1) * n_g_times) + (2 * N_bl * n_vis_times) + (N_ant * n_rfi_times)}"
)
...
true_params = {
    ...
    **{f"vis_r_induce": vis_ast_induce.real},
    **{f"vis_i_induce": vis_ast_induce.imag},
...
args.update(
    {
        ...
        "L_vis": jnp.array(
            [
                jnp.linalg.cholesky(
                    kernel(vis_ast_times, vis_ast_times, vis_var[i], vis_l[i])
                )
                for i in range(N_bl)
            ]
        ),
        ...
        "resample_vis": jnp.array(
            [
                resampling_kernel(vis_ast_times, times, vis_var[i], vis_l[i], 1e-6)
                for i in range(N_bl)
            ]
        ),
...
prior_mu = {
    ...
    **{"vis_r_induce": args["mu_vis_r"]},
    **{"vis_i_induce": args["mu_vis_i"]},
}
...
param_scale_tril = {
    ...
    **{f"vis_r_induce": args["L_vis"]},
    **{f"vis_i_induce": args["L_vis"]},
    **{
...
param_scale_tril_inv = {
    ...
    **{f"vis_r_induce": vmap(jnp.linalg.inv)(args["L_vis"])},
    **{f"vis_i_induce": vmap(jnp.linalg.inv)(args["L_vis"])},
    **{
        f"rfi_r_induce": jnp.broadcast_to(
