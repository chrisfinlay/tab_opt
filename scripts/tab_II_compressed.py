...
def initialize():
    ...
    args.update(
        {
            ...
            "L_G_amp": jnp.linalg.cholesky(
                kernel(g_times, g_times, g_amp_var, g_l, 1e-8)
            ),
            ...
            "resample_g_phase": resampling_kernel(
                g_times, times, g_phase_var, g_l, 1e-8
            ),
    ...
    def inv_transform(params, loc, inv_scaling):
        params_trans = {
            ...
            "g_phase_induce_base": vmap(
                affine_transform_full_inv, in_axes=(0, None, 0)
    ...
    mem_i += 1
    jax.profiler.save_device_memory_profile(f"memory_{N_ant:02}A_{mem_i}.prof")
    ...
    if fisher and rchi2 < 1.1:
        ...
        post_mean = (
            {k[:-9]: v for k, v in vi_params.items()} if opt else true_params_base
        )
    ...
    return model, args, v_obs_ri, init_params_base
...
model, args, v_obs, init_params_base = initialize()
...
def U(params):
    return (
        ...
        * log_density(
            model,
            model_args=(),
            model_kwargs={"args": args, "v_obs": v_obs},
            params=params,
        )[0]
    )
...
print("Calculate log_density")
history = {**q_i, "U": jnp.array([U(q_i) / v_obs.size])}
...
print("Calculate grad log_density")
grads = delU(q_i)
updates, opt_state = opt.update(grads, state)
q_new = optax.apply_updates(q_i, updates)
U_new = U(q_new) / v_obs.size

history = tree_map(
    lambda x, y: jnp.concatenate([x[None, :], y[None, :]], axis=0),
    history,
    {**q_new, "U": jnp.array([U_new])},
)

pbar = tqdm(range(200))
pbar.set_description(f"NL Posterior = {round(float(U_new), 3)}")


@jit
def get_update(q, history, opt_state):
    grads = delU(q)
    ...
    return q_new, history, opt_state


for _ in pbar:
    q_new, history, opt_state = get_update(q_new, history, opt_state)
    pbar.set_description(f"NL Posterior = {round(float(history['U'][-1]), 3)}")


plt.semilogy(history["U"])
plt.savefig("plots/AdaBelief.pdf", format="pdf")
